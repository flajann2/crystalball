 ...
 * LRCGraphics.java
 *
 * Created on January 15, 2002, 5:45 PM
 */
...
/**
 * A private graphics to access clip bounds without creating a new rectangle
 *
 * @author Arnaud Weber
 * @version 1.30 07/26/00
 */
class LRCGraphics extends Graphics implements LRCGraphicsWrapper {
    ...
    /* The LRCGraphics this object was cloned from, if any. */
    LRCGraphics previous;

    LRCGraphics(Graphics g) {
        if (g == null) {
            Thread.dumpStack();
        }
    ...
    void init(Graphics g) {
        if (g instanceof LRCGraphics) {
            /* Clone an existing LRCGraphics object.  The dispose method
             * must be called to reset the shared sub-graphics object to
             * the same state it was in when cloned. */
            LRCGraphics sg = (LRCGraphics) g;
            ...
            if (clipRect == null) {
                clipRect =
                    new Rectangle(sg.clipRect.x, sg.clipRect.y, sg.clipRect.width,
                                  sg.clipRect.height);
            } else {
            ...
            /* Initialize using a non-LRCGraphics Graphics object.  The
            ...
            if (cr == null) {
                if (clipRect == null) {
                    // Not a recycled LRCGraphics, allocate Rectangle.
                    clipRect = new Rectangle(0, 0, Integer.MAX_VALUE, Integer.MAX_VALUE);
                } else {
                    // Reuse recycled LRCGraphics' existing Rectangle.
             ...
             * only LRCGraphics wrappers.  We're therefore assuming
    ...
    /**
     * Create a LRCGraphics from another Graphics object, and set its clip to be the
     * intersection of the first Graphics object's clip rect. Graphics.create() normally
     * does this, but Microsoft's SDK for Java 2.0 doesn't set the clip of the returned
     * object.  Since this method is supposed to emulate what Graphics.create() does,
     * all potential bugs should be first checked with that method before changing the
     * behavior here.
     *
     * @param g DOCUMENT ME!
     * @param x DOCUMENT ME!
    ...
    public void translate(int x, int y) {
        ...
        if (TRACE) {
            System.out.println("translate: 0x"+Integer.toHexString(hashCode())+" x="+x
                               +", y="+y+", clipRect="+clipRect+" current translate "
    ...
    public boolean isClipIntersecting(Rectangle r) {
        if ((clipRect.x >= (r.x+r.width)) || ((clipRect.x+clipRect.width) <= r.x)
                || (clipRect.y >= (r.y+r.height))
                || ((clipRect.y+clipRect.height) <= r.y)) {
            return false;
        }
    ...
    private void resetGraphics() {
        if (TRACE) {
            System.out.println("resetGraphics: 0x"+Integer.toHexString(hashCode()));
        }
        ...
        if ((clipRect.x != previous.clipRect.x) || (clipRect.y != previous.clipRect.y)
                || (clipRect.width != previous.clipRect.width)
                || (clipRect.height != previous.clipRect.height)) {
            setClip(previous.clipRect.x, previous.clipRect.y, previous.clipRect.width,
                    previous.clipRect.height);
        }
    }

    public void dispose() {
        if (TRACE) {
            System.out.println("dispose: 0x"+Integer.toHexString(hashCode())+"("
                               +((previous == null) ? "null"
                                                    : Integer.toHexString(previous
                                                                          .hashCode()))
        ...
        graphics = null;
        LRCGraphics.recycleSwingGraphics(this);
    }
    ...
    public String toString() {
        ...
        return "LRCGraphics(0x"+Integer.toHexString(hashCode())+") [subGraphics "
               +originalGraphics.getClass().getName()+"\n   translate [x="+translateX
               +",y="+translateY+"] clip [x="+clipRect.x+",y="+clipRect.y+",w="
               +clipRect.width+",h="+clipRect.height+"]\n   color [r="
               +currentColor.getRed()+",g="+currentColor.getGreen()+",b="
               +currentColor.getBlue()+"] font "+fontString+"]";
    }
    ...
    private static void recycleSwingGraphics(LRCGraphics g) {
        synchronized (pool) {
            if (DEBUG) {
                if (pool.indexOf(g) != -1) {
    ...
    private static LRCGraphics getRecycledSwingGraphics() {
        LRCGraphics r = null;

        synchronized (pool) {
            if (pool.size() > 0) {
                r = (LRCGraphics) pool.pop();
            }
        }

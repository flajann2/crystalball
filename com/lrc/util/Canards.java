...
/**
 * The "Duck" threading class. Because executing some tasks may lead to deep
 * problems such as  deadly embrace, etc, and may not return, a dispatching
 * thread could be locked, thus halting all further dispatching.
 *
 * <p>
 * Canards allows you to splice off the task to seperate threads automatically,
 * in a fashion that is more efficient than just creating seperate threads.
 * Threads are created as needed and held in a queue until needed again. When a
 * task is completed, the thread is recovered and sits in the queue until needed
 * again. This way, creation of threads is minimized and are only done on a
 * as-needed basis.
 * </p>
 *
 * <p>
 * Also, job requests are queued, with the next available threads grabbing the
 * next entry off of the queue. There is no limit to the number of queueable
 * requests.
 * </p>
 *
 * <p>
 * If a thread does not return from its task within 60 seconds, it is said to be
 * "locked up", and thus will not be returned to the availability queue. A
 * throwable object may then be tossed into the locked thread to trigger a stack
 * dump that can be useful for debugging.
 * </p>
 *
 * <p>
 * Messages will be displayed on stdout and stderr if locked threads exceed the
 * time threshold. Also, if a thread tosses an exception, that exception is
 * caught and displayed too.
 * </p>
 *
 * <p></p>
 *
...
public final class Canards extends java.lang.Object {
        ...
        while (true) {
          ...
          for (int i = 0; i < oar.length; ++i) {
            ...
            if (stamp != null) {
              ...
              if (debug && (age > maxage))
                System.err.println("Canard warning: " + q.getName() + " is " +
                                   (age / 1000L) + " seconds old.");

              if (age > trashage) {
                System.err.println("Canard error: interrupting " + q.getName());
                q.stopRun(new CanardsException("Tossed Exception"));
                q.stamp = null;
              }
            }
          }
  ...
  public static void setMaxThreads(int max) { maxthreads = max; }
  ...
  public static boolean isDumping() { return dump; }

  public static int getReadyCount() { return cest.readyQueue.size(); }

  public static int getActiveCount() { return cest.activeQueue.size(); }

  public static int getJobCount() { return cest.jobQueue.size(); }

  String stat() {
    return "ready=" + getReadyCount() + ", active=" + getActiveCount() +
        ", jobs=" + getJobCount();
  }

  public static void run(Runnable r) { cest.runJob(r, false); }

  public static void runSerial(Runnable r) { cest.runJob(r, true); }
  ...
  private void runJob(Runnable r, boolean serialjob) {
    ...
    synchronized (readyQueue) {
      if (!readyQueue.isEmpty() && !serialjob) {
        ...
        if (q.r !=
            null) // we have a problem. We must scan queue for next avail.
        {
  ...
  class Quack extends Thread {
    ...
    Date stamp = null;
      ...
      public void stopRun(java.lang.Exception e) {
          die = true;
      }
      
    public void run() {
      synchronized (readyQueue) { readyQueue.addLast(this); }
      ...
      while (!die) {
          ...
          if (r == null)
            try {
              ...
              if (r != null) // if we have a job already...!?
                ...
                readyQueue.remove(
                    this); // then remove ourself from the ready list
            } catch (InterruptedException ie) {
            }
        }
        ...
        {
          synchronized (readyQueue) { readyQueue.remove(this); }

          synchronized (activeQueue) { activeQueue.addLast(this); }
          ...
          if (r == null) // if we have no more jobs to do...
          {
            ...
            synchronized (activeQueue) { activeQueue.remove(this); }
      ...
      System.out.println("Canads Thread:" + Thread.currentThread() +
                         " is dead.");
    }
  }
}
...
class CanardsException extends java.lang.Exception {
  CanardsException(String m) { super(m); }
}
